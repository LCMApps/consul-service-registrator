...
class ServiceRegistrator {
    ...
    register(overwrite) {
        if (!_.isEmpty(overwrite) && !_.isBoolean(overwrite)) {
            throw new Error('serviceName must be a string');
        }
        ...
        if (_.isArray(this._checks) && !_.isEmpty(this._checks)) {
            return this._deregisterIfNecessary(overwrite)
                .then(() => {
                    return new Promise((resolve, reject) => {
                                ...
                                this.deregister()
                                    .then(() => {
                                        reject(new Error(util.format(
                                            'Can not register one of checks for the service `%s`, ' +
                                            'failed with error: %s',
                                            this._serviceId,
                                            err
                                        )));
            ...
            return this._deregisterIfNecessary(overwrite).then(() => {
    ...
    _deregisterIfNecessary(isNecessary) {
        if (isNecessary) {
            return new Promise((resolve, reject) => {
                this._consul.agent.service.deregister(this._serviceId)
                    .then(resolve)
                    .catch(err => {
                        reject(new Error(util.format(
                            'Can not deregister service `%s`, failed with error: %s and ',
                            this._serviceId,
                            err
                        )));
                    });
