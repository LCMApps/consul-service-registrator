...
class ServiceObserver extends EventEmitter {
    constructor(options) {
        ...
        if (_.has(options, 'notifyNotesChange')) {
            if (_.isBoolean(options.notifyNotesChange)) {
                throw new DetailedError('options.notifyNotesChange must be a bool');
            }
        ...
        if (_.has(options, 'notifyOutputChange')) {
            if (_.isBoolean(options.notifyNotesChange)) {
                throw new DetailedError('options.notifyOutputChange must be a bool');
            }
    ...
    watch(serviceName, intervalSec, options = {}) {
        if (!_.isInteger(intervalSec)) {
            throw new DetailedError(
                util.format('intervalSec argument must be an int but %j was passed', typeof intervalSec)
            );
        }

        if (intervalSec < 1) {
            throw new DetailedError('intervalSec argument must be a non-zero positive integer', {intervalSec});
        }

        if (!_.isString(serviceName)) {
            throw new DetailedError(
                util.format('serviceName argument must be a string but %j was passed', typeof serviceName)
            );
        }

        if (!_.isObject(options)) {
            throw new DetailedError(util.format('options argument must be an array but %j was passed', typeof options));
        }
        ...
        if (this._checkOptions.has(serviceName)) {
            if (_.isEqual(this._checkOptions.get(serviceName), options)) {
                ...
                throw new DetailedError(
                    'Service is already registered with options that differs from passed',
                    {serviceName}
                );
            }
        }
    ...
    _repeatedCheck(serviceName, intervalSec) {
        ...
        if (!this._checkOptions.has(serviceName)) {
            this.emit('error', new DetailedError('checkOptions was not found', {serviceName}));
            return;
        }

        let options = this._checkOptions.get(serviceName);
        if (!_.isObject(options)) {
            this.emit('error', new DetailedError(
                util.format(
                    'checkOptions must be object but %s was found',
                    typeof options
                ),
                {serviceName}
            ));
        ...
        let promise = this._consul.health.service(options);
    ...
    _validateWatchOptions(options) {
        if (_.has(options, 'dc') && (!_.isString(options.dc) || _.isEmpty(options.dc))) {
            throw new DetailedError('options.dc must be a non-empty string');
        }

        if (_.has(options, 'tag') && (!_.isString(options.tag) || _.isEmpty(options.tag))) {
            throw new DetailedError('options.tag must be a non-empty string');
        }

        let unsupportedOptions = _.pull(_.keys(options), 'dc', 'tag', 'service');
        if (!_.isEmpty(unsupportedOptions)) {
            throw new DetailedError(util.format('options contains unsupported keys %j', unsupportedOptions));
        }
